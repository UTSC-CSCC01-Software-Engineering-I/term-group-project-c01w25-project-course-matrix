...
const TimetableBuilder = () => {
	...
	const courseEventsOfferingIds = [...new Set(eventsData?.courseEvents.map(
		(event) => event.offering_id
	))];
	const offeringIds = [...new Set([...courseEventsOfferingIds, ...(form.getValues("offeringIds") || [])])];
	...
	useEffect(() => {
		...
		const addedCourses = (eventsData?.courseEvents || []).map(
			(event) => event.event_name.split(" - ")[0].trim()
		);
		const newCourses = data?.filter((course: { code: string; }) => addedCourses.includes(course.code)) || [];
