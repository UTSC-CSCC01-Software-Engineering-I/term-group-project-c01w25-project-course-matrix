...
belong to the user. if (!timetableData || timetableData.length === 0) { return {
        status: 404,
  ...
  try {
    ...
    if (!timetable_title && !semester) {
      return {
        status: 400,
        error:
          "New timetable title or semester is required when updating a
timetable",
      };
    }
...
${offeringData.meeting_section} `; const courseDay = offeringData.day; const
courseStartTime = offeringData.start; const courseEndTime = offeringData.end;
      ...
      //Each week lecture will be inputted as a separate events from sememseter
start to end date
