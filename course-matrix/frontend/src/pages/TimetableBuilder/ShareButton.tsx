...
const ShareButton = ({calendar_id}: ShareButtonProps) => {
    ...
    const handleShare = async () => {
        ...
        const { error } = await shareTimetable({shared_email: email, calendar_id });
		if (error) {
	...
	return (
		<Dialog>
			...
			<TimetableSuccessDialog successMessage={successMessage} setSuccessMessage={setSuccessMessage} />
			...
			<DialogContent className="gap-5">
				<DialogHeader>
					...
					<DialogDescription>Who would you like to share your timetable with?</DialogDescription>
				</DialogHeader>
                <Label htmlFor="sharedEmail">Enter the email of the person you want to share your timetable with</Label>
                <Input id="sharedEmail" ref={emailRef} type="email" placeholder="Email" className="w-full" />
				<DialogFooter>
