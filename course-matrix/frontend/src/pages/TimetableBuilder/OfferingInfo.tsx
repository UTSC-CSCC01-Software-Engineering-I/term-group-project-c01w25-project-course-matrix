...
interface OfferingInfoProps {
	offeringIds: number[];
	isChoosingMeetingSectionsManually: boolean;
...
const OfferingInfo = ({
	offeringIds,
	isChoosingMeetingSectionsManually,
	course,
	semester,
	form,
}: OfferingInfoProps) => {
	...
	const initialSelectedLecture = selectedOfferingIds?.find((offering: { meeting_section: string; }) =>
		offering.meeting_section.startsWith("LEC")
	);
	const initialSelectedTutorial = selectedOfferingIds?.find((offering: { meeting_section: string; }) =>
		offering.meeting_section.startsWith("TUT")
	);
	const initialSelectedPractical = selectedOfferingIds?.find((offering: { meeting_section: string; }) =>
		offering.meeting_section.startsWith("PRA")
	);
	...
	const [selectedLecture, setSelectedLecture] = useState<OfferingModel | undefined>();
	const [selectedTutorial, setSelectedTutorial] = useState<OfferingModel | undefined>();
	const [selectedPractical, setSelectedPractical] = useState<OfferingModel | undefined>();
	...
	}, [initialSelectedLecture, initialSelectedTutorial, initialSelectedPractical]);
