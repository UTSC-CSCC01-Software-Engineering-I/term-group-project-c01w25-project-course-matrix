...
export const RestrictionSchema =
    z.object({
       ...
       disabled: z.boolean().optional(),
        ...
        .refine(
            (data) => {
              if (data.type && data.type === 'Restrict Day' && data.days &&
                  data.days.length > 4)
                return false;
              return true;
            },
        ...
        .refine(
            (data) => {
              if (data.type && data.type.startsWith('Restrict') && data.days &&
                  data.days.length < 1)
                return false;
              return true;
            },
        ...
        .refine(
            (data) => {
              if (data.type && data.type === 'Days Off' && data.numDays &&
                  data.numDays > 4)
                return false;
              return true;
            },
        ...
        .refine(
            (data) => {
              if (data.type && data.type === 'Days Off' &&
        ...
        .refine(
            (data) => {
              if (data.type && data.type === 'Restrict Before' &&
        ...
        .refine(
            (data) => {
              if (data.type && data.type === 'Restrict After' &&
...
export const TimetableFormSchema: ZodType<TimetableForm> =
    z.object({
       name: z.string()
       ...
       restrictions: z.array(RestrictionSchema),
        ...
        .refine(
            (data) => {
              return !(
                  data.restrictions.filter((r) => r.type === 'Days Off')
                      .length > 1);
            },
            {
