...
import {categorizeValidOfferings, getFreq, getMaxDays, getMaxHour, getValidOfferings, groupOfferings, trim,} from '../utils/generatorHelpers';
...
export const availableFunctions: AvailableFunctions = {
  getTimetables: async (args: any, req: Request) => {
    try {
      ...
      let timeTableQuery = supabase.schema('timetable')
                               .from('timetables')
                               .select()
                               .eq('user_id', user_id);
  ...
  updateTimetable: async (args: any, req: Request) => {
    try {
      ...
      const {data: timetableUserData, error: timetableUserError} =
          await supabase.schema('timetable')
              .from('timetables')
              .select('*')
              .eq('id', id)
              .eq('user_id', user_id)
      ...
      let updateTimetableQuery = supabase.schema('timetable')
                                     .from('timetables')
                                     .update(updateData)
                                     .eq('id', id)
                                     .eq('user_id', user_id)
  ...
  deleteTimetable: async (args: any, req: Request) => {
    try {
      ...
      let deleteTimetableQuery = supabase.schema('timetable')
                                     .from('timetables')
                                     .delete()
                                     .eq('id', id)
                                     .eq('user_id', user_id);
...
timetable.`,
      ...
      let insertTimetable = supabase.schema('timetable')
                                .from('timetables')
                                .insert([
                                  {
                                    user_id,
...
${offeringData.meeting_section} `;
  ...
  getCourses: async (args: any, req: Request) => {
    ...
    try {
      ...
      const {data, error} =
          await supabase.schema('course').from('courses').select('*').or(
