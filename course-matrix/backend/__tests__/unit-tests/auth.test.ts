...
jest.mock(
    ...
    () => ({
      ...
      session: jest.fn((req: Request, res: Response) => {
        ...
        res.status(200).json(
            {message: 'Session valid', user: {email: 'validUser'}});
      }),
...
describe('Authentication API', () => {
  describe('POST /auth/signup', () => {
    it('should return 201 and a success message for valid signup data',
       async () => {
         ...
         expect(response.body.user)
             .toHaveProperty('email', 'newUser@example.com');
  ...
  describe('POST /auth/login', () => {
    ...
    it('should return 400 if email or password is missing', async () => {
      const response =
          await request(app).post('/auth/login').send({email: 'validUser'});
      expect(response.status).toBe(400);
      expect(response.body)
          .toHaveProperty(
              'error',
  ...
  describe('POST /auth/request-password-reset', () => {
    it('should return 200 and a success message when email is provided',
       async () => {
         const response =
             await request(app).post('/auth/request-password-reset').send({
  ...
  describe('POST /auth/reset-password', () => {
    it('should return 200 and a success message when password and token are provided',
       async () => {
  ...
  describe('DELETE /auth/accountDelete', () => {
    it('should return 200 and a success message when userId is provided',
       async () => {
         const response =
             await request(app).delete('/auth/accountDelete').send({
               userId: '12345'
             });
         expect(response.status).toBe(200);
         expect(response.body)
