...
describe('POST /api/timetables', () => {
  ...
  test(
      'should return error code 400 and error message: \'timetable title and semester are required\' when request body miss timetable_title or semester',
      async () => {
        const user_id = USER3;
        const newTimetable = {};
        ...
        const response =
            await request(app).post('/api/timetables').send(newTimetable);
        // Check database interaction and response
  ...
  test(
      'should create a new timetable given timetable title, timetable semester, timetable favorite',
      async () => {
        const user_id = USER3;
        const newTimetable = {
  ...
  test(
      'should return error code 400 and error message: A timetable with this title already exists when checkduplicate query return a value',
      async () => {
        const user_id = USER4;
        const newTimetable = {
          timetable_title: 'Minh timetable',
...
describe('PUT /api/timetables/:id', () => {
  ...
  test('should update the timetable successfully', async () => {
    ...
    const response =
        await request(app).put('/api/timetables/1').send(timetableData);
