...
export default {
  ...
  createRestriction: asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      if (!['Restrict Day', 'Days Off', 'Max Gap'].includes(type) &&
          !start_time && !end_time) {
      ...
      if (user_id !== timetable_user_id) {
        return res.status(401).json(
            {error: 'Unauthorized access to timetable restriction'});
      }
    ...
    try {
      ...
      if (updateData.start_time) {
        ...
        updateData.start_time =
            restriction_start_time.toISOString().split('T')[1];
      }
  ...
  deleteRestriction: asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const {error: restrictionError} = await supabase.schema('timetable')
                                            .from('restriction')
                                            .delete()
                                            .eq('user_id', user_id)
                                            .eq('calendar_id', calendar_id)
