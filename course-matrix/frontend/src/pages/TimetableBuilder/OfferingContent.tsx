...
const OfferingContent = ({
  ...
  const { data, isLoading, error, refetch } = useGetOfferingsQuery({course_code: item.code, semester: semester})
        ...
        <Table className="w-[1000px] text-xs">
          ...
          <TableBody>
            {(data as OfferingModel[]).map((offering, index) => ( 
              <TableRow>
                <TableCell className="font-medium">{offering.meeting_section}</TableCell>
