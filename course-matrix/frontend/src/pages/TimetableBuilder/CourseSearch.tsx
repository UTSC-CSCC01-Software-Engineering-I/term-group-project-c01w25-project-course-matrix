  ...
  return (
    <div className="relative w-full">
      ...
      <FilterIcon
        size={16}
        onClick={showFilter}
        className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-800 transition cursor-pointer"
      />
      {showPanel && (
        <div
          ref={panelRef}
          className="absolute top-full left-0 w-full bg-white shadow-md border rounded-md mt-2 p-2 z-[100] max-h-[500px] overflow-y-auto"
        >
          {data && data.length > 0 ? (
            data.map((item, index) => (
              <div
                    ...
                    <div className="text-sm flex flex-col gap-2">
                      ...
                      <div>
                        <p>
                          ...
                          {convertBreadthRequirement(
                            item.breadth_requirement ?? "",
                          )}
                        </p>
                        ...
                        {item.recommended_preperation && (
                          <p>
                            <strong>Recommended Preperation: </strong>{" "}
                            {item.recommended_preperation}
                          </p>
                        )}
                        {item.note && (
